...
public:
    ...
    //!
    //! \brief Add a calibration profile.(校准配置)
    //!
...
{
    //! Mark the network to be an explicit batch network.							明确的
    //! Dynamic shape support requires that the kEXPLICIT_BATCH flag is set.		kEXPLICIT_BATCH = 1，动态批处理
    //! With dynamic shapes, any of the input dimensions can vary at run-time,		意思是，如果输入的维度在运行时是变化的，那就需要将kEXPLICIT_BATCH设置为1
    ...
    //! Deprecated. This flag has no effect now, but is only kept for backward compatability. 现在无效
    //!
...
public:
    ...
    //! The builder retains ownership(所有权) of the created optimization profile and returns a raw pointer, i.e. the users
